...
RETf ADD(const __m128 x, const float y) { return ADD(x, SET(y)); }
RETf SUB( const __m128 x, const __m128 y ) { return _mm_sub_ps(x,y); }
RETf SUB(const __m128 x, const __m128 y) { return _mm_sub_ps(x, y); }
RETf SUB(const __m128 x, const float y) { return SUB(x, SET(y)); }

RETf NEG(const __m128 x)				 { return SUB(SET(0.f),x); }
...
RETf ATAN(const __m128 x) { return _mm_atan_ps(x);}
...
RETf _MIN( const __m128 x, const __m128 y ) { return _mm_min_ps(x,y); }
